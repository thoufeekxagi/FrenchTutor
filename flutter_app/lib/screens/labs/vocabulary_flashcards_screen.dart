...
class VocabularyFlashcardsScreen extends ConsumerStatefulWidget {
  const VocabularyFlashcardsScreen({
    ...
    this.coverUrl,
    @Deprecated(
      'Audio is generated by the owning lesson, never on this screen.',
    )
    this.prefetchAudio = false,
  ...
  /// Kept only for source compatibility with older callers. It is ignored.
...
class _VocabularyFlashcardsScreenState
  ...
  /// The Live pronunciation checker is opt-in. The switch reflects an active
  /// socket, not merely a persisted preference, so backgrounding or leaving
  /// the route visibly turns it off and cannot leave the learner thinking a
  /// billable call is still open.
  ...
  Future<void> _setMurrayEnabled(bool enabled) async {
    ...
    if (!enabled) {
      ...
      await _murray.end();
      await settings.setEnabled(TutorHelperSurface.vocabulary, false);
      if (!mounted) return;
      setState(() {
        _recording = false;
    ...
    await settings.setEnabled(TutorHelperSurface.vocabulary, true);
    if (!mounted) return;
    setState(() => _audioError = null);
    await _murray.start(context, sendOpeningPrompt: false);
    if (!mounted) return;
    if (!_murray.isLive) {
      // Do not leave the switch visually on after a failed connection. The
      // learner can retry explicitly, and the failed attempt cannot retry in
      // the background.
      await settings.setEnabled(TutorHelperSurface.vocabulary, false);
      setState(() {
        _audioError = _murray.error ?? 'Marie could not connect.';
      });
    }
    setState(() {});
  }
  ...
  /// Prepare only missing text context for a custom set. Audio is never
  ...
  void _resolveAttempt(String transcript, {required bool sentence}) {
    ...
    final matches =
        heard.isNotEmpty && (heard.contains(wanted) || wanted.contains(heard));
    _murrayTurnClosing = false;
    setState(() {
      if (sentence) {
        _sentenceRecording = false;
