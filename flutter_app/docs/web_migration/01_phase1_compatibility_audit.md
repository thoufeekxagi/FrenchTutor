...
app, so Phases 3-6 have no surprises. This phase made no code changes.
...
- `lib/data/database/database_opener.dart` already conditionally exports a native vs. web SQLite opener
  ...
  loads `sqlite3.wasm` + `IndexedDbFileSystem` correctly). `web/sqlite3.wasm` is already checked in. **Phase 4
...
|---|---|---|---|
| `drift` + `database_opener.dart`/`database_opener_web.dart` | native `sqlite3` via `database_opener_native.dart` | **Already implemented**: loads `sqlite3.wasm`, uses `IndexedDbFileSystem` | Verify only — see Phase 4 |
...
| `lib/services/revenue_cat_service.dart` | native IAP via `purchases_flutter`, gated by `Platform.isIOS`/`Platform.isAndroid` | Already gated off (`isConfigured` false on web) — safe, but means **no entitlement path exists on web at all yet** | Real new code — see Phase 3 (Stripe/RevenueCat Web Billing) |
| `lib/screens/settings/settings_screen.dart` `_openManageSubscriptions()` | deep-links to App Store/Play Store subscription management via `Platform.isIOS` | needs a web branch — link to whatever web billing portal Phase 3 picks (e.g. Stripe customer portal), or hide the row until Phase 3 lands | Shim |
| `google_sign_in` | native SDK sheet | package has a web implementation; flow differs (renders its own button/redirect) | Shim — see Phase 3 |
| `sign_in_with_apple` | native SDK sheet | needs Apple's web "Sign in with Apple JS" + a registered Service ID/redirect URI, real setup work | Shim, but with real config overhead — Phase 3 should explicitly decide whether this ships in web v1 |
| `permission_handler` | native permission dialogs | browser permission prompts; only the mic permission actually matters here (used by `audio_streaming_service.dart`) | Shim, low effort, tied to Phase 5 |
...
- One cosmetic-only artifact observed: this sandboxed browser has no WebGL, so Flutter's web renderer fell
  ...
  Phase 6 before treating it as an actual issue.
...
One follow-up still worth doing early in Phase 4: explicitly exercise the `database_opener_web.dart` path
