...
Future<List<int>> _generate(
  ...
  const maxAttempts = 4;
  // Keep a real French transcript in the request. The selected Gemini TTS
  // model receives it through OpenRouter and returns raw PCM bytes.
  final transcript = _ttsTranscript(audioId, spokenName);
  final prompt = '[clear, careful, isolated French pronunciation] $transcript.';
...
String _ttsTranscript(String audioId, String spokenName) => switch (audioId) {
  ...
  'alphabet_letter_E' => 'le',
  'alphabet_letter_I' => 'ici',
  'alphabet_letter_O' => 'eau',
  'alphabet_letter_U' => 'tu',
  'alphabet_accent_e_acute' => 'aigu',
  'alphabet_accent_e_grave' => 'père',
  'alphabet_accent_e_circumflex' => 'forêt',
  'alphabet_accent_c_cedilla' => 'garçon',
  'alphabet_accent_e_diaeresis' => 'Noël',
  _ => spokenName,
};
